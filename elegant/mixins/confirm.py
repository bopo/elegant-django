...
from ..constants import CONFIRM_CHANGE
from ..constants import CONFIRMATION_RECEIVED
...
class AdminConfirmMixin:
    ...
    def get_confirmation_fields(self, request, obj=None):
        ...
        model_fields = {field.name for field in self.model._meta.fields}  # noqa
        admin_fields = set(flatten_fieldsets(self.get_fieldsets(request, obj)))  # noqa
    ...
    def render_change_confirmation(self, request, context):
        ...
        context.update(media=self.media, )  # noqa
    ...
    def _add_confirmation_options_to_extra_context(self, extra_context):
        ...
        return {**(extra_context or {}), 'confirm_add': self.confirm_add, 'confirm_change': self.confirm_change}

    def _get_changed_data(self, form: ModelForm, model: Model, obj: object, add: bool) -> dict:  # noqa
        """
        ...
        model - the model class of the obj # noqa
        ...
        def _display_for_changed_data(field, initial_value, new_value):  # noqa
        ...
        if add:
            for name, new_value in form.cleaned_data.items():
                ...
                field_object = model._meta.get_field(name)  # noqa
    ...
    def _confirmation_received_view(self, request, object_id, form_url, extra_context):
        ...
        ie. There is no way to set a file on the html form # noqa
        ...
        def _reconstruct_request_files():
            ...
            if type(cached_object) != self.model:  # noqa
                # Do not use cache if the model doesn't match this model
                log(f'Warning: cached_object is not of type {self.model}')  # noqa
                return

            query_dict = request.POST

            for field in self.model._meta.get_fields():  # noqa
                if not (isinstance(field, FileField) or isinstance(field, ImageField)):
                    continue
        ...
        if reconstructed_files:
            ...
            if object_id and SAVE_AS_NEW not in request.POST:
                ...
                obj = self.model.objects.filter(id=object_id).first()  # noqa
            else:
        ...
        return super()._changeform_view(request, object_id, form_url, extra_context)  # noqa

    def _get_cleared_fields(self, request):  # noqa
        """
    ...
    def _change_confirmation_view(self, request, object_id, form_url, extra_context):
        ...
        if to_field and not self.to_field_allowed(request, to_field):  # noqa
        ...
        if add:
            if not self.has_add_permission(request):  # noqa
                raise PermissionDenied

            obj = None
        else:
            obj = self.get_object(request, unquote(object_id), to_field)  # noqa

            if obj is None:
                return self._get_obj_does_not_exist_redirect(request, opts, object_id)  # noqa

            if not self.has_view_or_change_permission(request, obj):  # noqa
                raise PermissionDenied

        fieldsets = self.get_fieldsets(request, obj)  # noqa
        ModelForm = self.get_form(request, obj, change=not add, fields=flatten_fieldsets(fieldsets))  # noqa

        form = ModelForm(request.POST, request.FILES, instance=obj)
        form_validated = form.is_valid()

        if form_validated:
            new_object = self.save_form(request, form, change=not add)  # noqa
        else:
            new_object = form.instance

        formsets, inline_instances = self._create_formsets(request, new_object, change=not add)  # noqa
        ...
        context = {
            **self.admin_site.each_context(request),  # noqa
...
def confirm_action(func):
    ...
    def func_wrapper(modeladmin, request, queryset):
        ...
        affected_objects = queryset if isinstance(queryset, list) else [queryset]
